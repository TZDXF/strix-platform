...
import {
  btn, btnGhost, card, hint, h3, input, label, tableTd, tableTh,
} from '../ui'
...
async function create() {
  ...
  try {
    ...
    toast.success(`用户 ${form.value.username} 已创建`)
...
async function removeUser(u: User) {
  ...
  try { await api.deleteUser(u.id); toast.success(`用户 ${u.username} 已删除`); refresh() } catch (e) { toast.error((e as Error).message) }
}
...
async function saveReset() {
  if (resetPwd.value.length < 8) { toast.error('密码至少 8 位'); return }
  if (!resetFor.value) return
  try {
    await api.patchUser(resetFor.value.id, { password: resetPwd.value })
    toast.success(`已重置 ${resetFor.value.username} 的密码`)
...
<template>
  <div :class="card">
    <div class="mb-3.5 flex items-center gap-2.5">
      ...
      <button :class="btn" @click="showCreate = true">创建用户</button>
    </div>
    ...
    <DialogRoot v-model:open="showCreate">
      <DialogPortal>
        ...
        <DialogContent class="fixed top-1/2 left-1/2 z-50 max-h-[85vh] w-[520px] max-w-[94vw] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[10px] border border-border bg-panel p-5">
          <DialogTitle class="mb-4 text-base font-semibold text-text">创建用户</DialogTitle>
          ...
          <div class="mt-4 flex items-center gap-2.5">
            <div class="flex-1"></div>
            <button :class="btnGhost" @click="showCreate = false">取消</button>
